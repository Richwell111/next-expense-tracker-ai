...
function getOpenAI(): OpenAI {
  if (!openaiClient) {
    ...
    if (!apiKey) {
      console.warn(
        'Warning: API key not configured. AI features will not work. Please set OPENROUTER_API_KEY or OPENAI_API_KEY in your .env file.'
      );
      throw new Error(
        'API key not configured. Please set OPENROUTER_API_KEY or OPENAI_API_KEY environment variable.'
      );
    }
